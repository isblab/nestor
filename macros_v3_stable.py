...
class NestedSampling():
    ...
    def parse_likelihoods(self,iteration,fhead='likelihoods_'):
        ...
        if math.nan in sampled_likelihoods:
            ...
            self.terminator(iteration=iteration, conv_hits=self.stopper_hits, es_hits=self.es_counter, worst_likelihood=0)
        
        return sampled_likelihoods


    def check_stopper(self,iteration,es_hits,worst_likelihood):
        '''
        ...
        if ere_val < self.ere_threshold:
            ...
            if self.stopper_hits == self.stopper_cap:
                self.termination_mode = 'Convergence'
                self.finished = True
    ...
    def terminator(self,iteration,conv_hits,es_hits,worst_likelihood):
        if not 'error' in self.termination_mode.lower():
            ...
            with open("estimated_evidence.dat",'a') as eedat:
                eedat.write(f"{self.Z}\n")

            from matplotlib import pyplot as plt
            fig,ax = plt.subplots(1)
            ax.plot(self.log_xi,self.log_worst_likelihoods)
        ...
        with open("run.log",'w') as rlf:
            ...
            rlf.write(f"Current convergence criterion hits: {conv_hits}\n") 
            rlf.write(f"Current early stopper_hits: {es_hits}\n")
    ...
    def compute_evidence(self,iteration,worst_likelihood):
        self.curr_xi = math.exp(-iteration/self.num_init_frames)
        self.wi = self.Xi - self.curr_xi
        self.new_Z = worst_likelihood * self.wi

        self.Xi = self.curr_xi 
        self.Z += self.new_Z
    ...
    #         self.terminator(mode=self.termination_mode,iteration=iteration, conv_hits=self.stopper_hits, es_hits=self.es_counter)
    ...
    def execute_nested_sampling2(self):
        ...
        if not 'shuffle_error.log' in os.listdir('./'):
            ...
            while true_iter < self.nester_niter+self.num_init_frames:
                if 'run.log' in os.listdir('./'):
                    break
                ...
                if base_process:
                    ...
                    if not self.finished:
                        ...
                        candidate_li = min(self.likelihoods)
                    ...
                    if candidate_li >= Li:
                        ...
                        if true_iter>1:
                            self.compute_H(new_li=Li, new_wi=self.wi, new_zi=self.new_Z+self.Z, old_zi=self.Z)
                        ...
                        if i>1 and not self.finished:
                            self.check_stopper(iteration=true_iter,es_hits=self.es_counter, worst_likelihood=Li)
                        ...
                        if self.es_counter == self.es_limit:
                            self.termination_mode = 'EarlyStopper' 
                            self.finished = True
                    
                    true_iter += 1        
                    print(f'\n-----> True iteration: {true_iter} {" "*5} Calculation iteration: {i} {" "*5} ES_counter: {self.es_counter} {" "*5} Evidence: {self.Z} {" "*5} Terminating: {self.finished}\n')
                    if self.finished:
                        if base_process and self.finished:
                            with open('fin.tmp','w') as ftmp:
                                ftmp.write('Finishing')
                
                if base_process and len(self.likelihoods)==0:
                    self.terminator(iteration=true_iter, conv_hits=self.stopper_hits, es_hits=self.es_counter, worst_likelihood=Li)
                self.comm_obj.Barrier()

            if true_iter==self.nester_niter:
                self.termination_mode = 'MaxIterations'
                self.terminator(iteration=true_iter, conv_hits=self.stopper_hits, es_hits=self.es_counter, worst_likelihood=Li)
            
        else:
            self.termination_mode = 'Error: Shuffle configuration error'
            self.terminator(iteration=0,conv_hits=0,es_hits=0, worst_likelihood=0)
