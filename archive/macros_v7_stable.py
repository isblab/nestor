...
class ReplicaExchange0(object):
    ...
    def __init__(
        ...
        if (
            isinstance(root_hier, IMP.atom.Hierarchy)
        ...
        if not self.test_mode and not self.nest:
            ...
            if not self.is_multi_state:
                if self.vars["number_of_best_scoring_models"] > 0:
                    ...
                    output.write_psf(
                        pdb_dir + "/" + "model.psf",
                        pdb_dir + "/" + self.vars["best_pdb_name_suffix"] + ".0.pdb",
                    )
            else:
                if self.vars["number_of_best_scoring_models"] > 0:
                    for n in range(self.vars["number_of_states"]):
...
class NestedSampling:
    def __init__(self, h_param_file, nester_restraints, rex_macro, exit_code):
        import yaml
    ...
    def terminator(self, iteration, plateau_hits, failed_iter, worst_likelihood):
        ...
        if not "error" in self.termination_mode.lower():
            ...
            #     rlf.write(f"Analytical uncertainty: {ana_unc}\n")
        ...
        for ncl, n1 in enumerate(cluster.members):
            ...
            if not consolidate:
                ...
                for name1, name2 in binary_dists_dict:
                    ...
                    binary_dists[
                        min(r1) : max(r1) + 1, min(r2) : max(r2) + 1
