...
class NestedSampling():
    def __init__(self, h_param_file, nester_restraints, rex_macro):
        import yaml
        ...
        self.nester_niter = self.h_params['max_nester_iter']
    ...
    def terminator(self,iteration,plateau_hits,failed_iter,worst_likelihood):
        ...
        if not 'error' in self.termination_mode.lower():
            ...
            with open("run.log",'w') as rlf:
                rlf.write(f"Run paramerters: {self.h_params}\n\n")
                ...
                rlf.write(f"Nestor process time: {self.toc - self.tic} seconds\n")
    ...
    def execute_nested_sampling2(self):
        ...
        if not 'shuffle_error.log' in os.listdir('./'):
            ...
            if not 'error.log' in os.listdir('./'):
                if base_process:
            ...
            while true_iter < self.nester_niter :
                ...
                if 'run.log' in os.listdir('./') or 'error.log' in os.listdir('./'):
            ...
            if base_process and 'fin.tmp' in os.listdir('./'):
                os.remove('fin.tmp')
        ...
        if base_process:
            if not 'error' in self.termination_mode.lower():
                ...
                if self.termination_mode=='Error: Nan found' or self.termination_mode=='Error: Shuffle configuration error':
                    self.exit_code = 11
                elif self.termination_mode=='Error: MaxIterations reached without convergence criteria': 
                    self.exit_code = 12
